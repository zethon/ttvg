 ...
 *
 */
ItemPtr ItemFactory::createItem(const ItemInstanceInfo& instinfo)
{
...
ItemPtr ItemFactory::createItem(const std::string& objid)
{
    ...
    auto item = std::make_shared<Item>(objinfo, ItemInstanceInfo{});
    return item;
}

ItemInfo& ItemFactory::getObjectInfoRef(const std::string& objid)
{
    ...
    ItemInfo retval = j.get<tt::ItemInfo>();
    retval.id = objid;
